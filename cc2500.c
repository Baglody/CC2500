...
//		CC2500_WriteReg(CCxxx0_MDMCFG2,	0x70);
		...
		CC2500_WriteReg(CCxxx0_FREND1,	0x56);								// 0xB6   56´íÎó»úÂÊ¼õÉÙ
		...
		CC2500_WriteReg(CCxxx0_FOCCFG,	0x15);								// 0x1D   ÆµÂÊÆ«ÒÆ²¹³¥
		CC2500_WriteReg(CCxxx0_BSCFG,	0x6C);								// 0x1c   Î»Í¬²½ÅäÖÃ
		CC2500_WriteReg(CCxxx0_AGCCTRL2,	0x07);								// 0xc3    ÔöÒæ¿ØÖÆ
		...
		CC2500_WriteReg(CCxxx0_FSCAL1,	0x00);								// Ôö¼ÓÆµÂÊÍ¬²½Ð£×¼
		...
		CC2500_WriteReg(CCxxx0_PKTCTRL1,	0x00);	//ÎÞµØÖ·¼ì²é		//                      ??????
...
void CC2500_SetRecvAddr(CC2500_RECV_ADDR AddrMode,u8 SpecAddr)
{
	...
	CC2500_WriteReg(CCxxx0_PKTCTRL1,AddrMode);//µØÖ·¼ì²é¼Ä´æÆ÷
...
bool CC2500_Init(void)
{
	...
	EXTI_InitStructure.EXTI_Line = EXTI_Line3;             //Íâ²¿ÖÐ¶ÏÏß £¬Ê¹ÓÃµÚ2¸ù
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;    //ÖÐ¶ÏÄ£Ê½
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;//ÖÐ¶Ï´¥·¢·½Ê½
	EXTI_InitStructure.EXTI_LineCmd = ENABLE;              //´ò¿ªÖÐ¶Ï
	EXTI_Init(&EXTI_InitStructure);    //µ÷ÓÃ¿âº¯Êý¸ø¼Ä´æÆ÷¸´ÖÆ
	...
	EXTI_InitStructure.EXTI_Line = EXTI_Line6;             //Íâ²¿ÖÐ¶ÏÏß £¬Ê¹ÓÃµÚ2¸ù
	EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;    //ÖÐ¶ÏÄ£Ê½
	EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;//ÖÐ¶Ï´¥·¢·½Ê½
	EXTI_InitStructure.EXTI_LineCmd = ENABLE;              //´ò¿ªÖÐ¶Ï
	EXTI_Init(&EXTI_InitStructure);    //µ÷ÓÃ¿âº¯Êý¸ø¼Ä´æÆ÷¸´ÖÆ
...
#if QXW_LCM_ID == 210 || QXW_LCM_ID == 211 || QXW_LCM_ID == 220 || QXW_LCM_ID == 221
	if(Enable)
		EXTI->IMR |= EXTI_Line3;//¿ªÆôÖÐ¶Ï
	else
		EXTI->IMR &=~EXTI_Line3;//¹Ø±ÕÖÐ¶Ï
#elif QXW_LCM_ID == 212
	if(Enable)
		EXTI->IMR |= EXTI_Line6;//¿ªÆôÖÐ¶Ï
	else
		EXTI->IMR &=~EXTI_Line6;//¹Ø±ÕÖÐ¶Ï
#endif
}
...
{ 
	...
	CC2500_SendSpiData(CCxxx0_SIDLE);
	CC2500_SendSpiData(CCxxx0_SFTX);//SFRX SFTXÖ»ÓÃÓÚIDLEÄ£Ê½ Ö´ÐÐºó½«FIFOsÀïÉÏÒç»òÏÂÒçµÄÊý¾ÝÇå¿Õ
...
u8 CC2500_ReceivePacket(u8 *rxBuffer)
{
    ...
    status =CC2500_ReadStatus(CCxxx0_RXBYTES);          //RX FIFO µÈÓÚ0 CRCÐ£ÑéÊ§°Ü Èç¹û´óÓÚ0CRCÐ£ÑéOKÊý¾ÝÄÜ¶Á³ö
    if((status & 0x7F)==0x00)
    {
        status = CC2500_ReadStatus(CCxxx0_MARCSTATE);
        if((status!=0x0D)&&(status!=0x08))    //0x0DÎªRX×´Ì¬
        {
        	CC2500_SetRxd();				//¼¤»î£¨½ÓÊÕ»ò´«ËÍ£©
        }
        return 0;
    }
	
	//LEN = CC2500_ReadReg(CCxxx0_RXFIFO_one);
    LEN = PACKET_LEN;   //²ÉÓÃ¶¨³¤Ä£Ê½
    if(LEN > 0)
    {
...
void CC2500_Init_WOR(void)
{
    ...
    CC2500_WriteReg(CCxxx0_MCSM0, 0x38);  // ÆµÂÊÐ£×¼·½Ê½£¬Ã¿4´Î´ÓIDLEÄ£Ê½µ½RXÄ£Ê½½øÐÐÒ»´ÎÆµÂÊºÏ³ÉÆ÷Ð£×¼
